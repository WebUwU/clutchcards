...
export async function POST() {
  ...
  if (!process.env.HENRIK_API_KEY || !account.puuid) {
    // Without a key we can't fetch; report gracefully (no fake data).
    await prisma.valorantAccount.update({ where: { userId: u.userId }, data: { lastSync: new Date() } });
    return fail("Match sync needs a server HENRIK_API_KEY and a verified account.", 503);
  }
  ...
  try {
    ...
    for (const m of matches) {
      const row = extractPlayerMatch(m, account.puuid);
      if (!row) continue;
